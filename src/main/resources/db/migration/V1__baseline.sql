-- V1 基线：全量当前 schema（可移植 DDL，H2 MODE=MySQL 与 MySQL 8.0 均兼容）
-- 来源：resources/sql/h2-schema.sql（当前唯一权威可移植 schema）。全幂等：
--   - CREATE TABLE IF NOT EXISTS：已有库（有表无 flyway history）baseline 后重跑为 no-op；
--   - 种子数据用 ON DUPLICATE KEY UPDATE：避免 H2 专属 MERGE、MySQL 重复键报错；
--   - 唯一索引全部内联进 CREATE TABLE，避免 MySQL 不支持的 CREATE INDEX IF NOT EXISTS。
...
-- 幂等种子：已有行 ON DUPLICATE KEY UPDATE id=id 为 no-op
...
create table if not exists article_quality (
    ...
    constraint uq_aq_task_score_type unique (task_id, score_type)
);
