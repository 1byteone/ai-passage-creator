...
public interface PromptConstant {
     ...
     * 智能体1：生成单个标题（用于完整流程）
     */
    String AGENT1_SINGLE_TITLE_PROMPT = """
        你是一位爆款文章标题专家,擅长创作吸引人的标题。
        
        根据以下选题,生成 1 个最佳的爆款文章标题:
        选题：{topic}
    ...
    String AGENT2_OUTLINE_PROMPT = """
        ...
        4. 适合2000字左右的文章
        5. **重要：必须返回完整、合法的 JSON 格式，不要截断**
        6. **重要：points 数组中的所有字符串必须用双引号包裹，且数组必须正确闭合**
        7. **重要：JSON 的最后必须有正确的闭合括号 }} 和 ]**
